...
public class RouterManager {
    ...
    //LruCache  key:类名 value: 路由组Grop的加载接口
    private LruCache<String, ARouterLoadGroup> groupCache;
    //LruCache  key:类名 value: 路由Path路径的加载接口
    ...
    private RouterManager() {
        groupCache = new LruCache<>(10);
        pathCache = new LruCache<>(10);
    }
    ...
    public Object navigation(Context context, BundleManager bundleManager, int code) {
        // 注意!!：阿里的路由path随意写，导致无法找到随意拼接APT生成的源文件，如：ARouter$$Group$$abc
        ...
        Log.e("frizzle >>> ", "groupClassName -> " + groupClassName);
        //读取路由组的Group类文件 (缓存,懒加载)
        try {
            ARouterLoadGroup groupLoad = groupCache.get(group);
            if (groupLoad == null) {
                //加载APT路由组Group类文件ARouter$$Group$$order
