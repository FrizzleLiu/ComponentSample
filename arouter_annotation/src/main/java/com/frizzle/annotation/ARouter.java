package com.frizzle.annotation;
 ...
 * 1、RUNTIME :一般如果需要在运行时去动态获取注解信息，用RUNTIME注解
 * 2、CLASS :要在编译时进行一些预处理操作，如ButterKnife，用CLASS注解。注解会在class文件中存在，但是在运行时会被丢弃
 * 3、SOURCE :做一些检查性的操作，如@Override，用SOURCE源码注解。注解仅存在源码级别，在编译的时候丢弃该注解
 */
